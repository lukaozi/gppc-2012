...
public:
	...
	SubgoalGraph(std::vector<bool> &bits, int width, int height, const char *filename, double memoryLimit = MEMORY_LIMIT, double timeLimit = TIME_LIMIT);
	...
	void MemoryAnalysis(double memoryLimit = MEMORY_LIMIT);	// Decide if we have extra space for pairwise distances 
	...
	bool IsLookaheadHReachable	// Similar to QuickHReachable, better chance of finding a path (some extra cost)
		(xyLoc & from, xyLoc & to, std::vector<mapLoc> & path = defaultPath, bool append = false);
	void GetSafeHReachableSubgoals	// Given a location, find all the subgoals it should be connected to
	...
	cost TryLocalPair	// Try to find a local subgoal pair connecting start and goal (not guaranteed)
		(subgoalId & sgStart, subgoalId & sgGoal, std::vector<subgoalId> & startSafeHReachableSubgoals, std::vector<subgoalId> & goalSafeHReachableSubgoals, std::vector<subgoalId> & localPath);
	...
	cost CheckAllLocalPairs	// Try to find a local subgoal pair connecting start and goal (guaranteed)
		(subgoalId & sgStart, subgoalId & sgGoal, std::vector<subgoalId> & startSafeHReachableSubgoals, std::vector<subgoalId> & goalSafeHReachableSubgoals, std::vector<subgoalId> & localPath);			
	void AppendOptimalPathRec	// Not being used (use the next pointers to recursively build the subgoal path)
	...
	cost LookupOptimalPath	// Main function for looking up pairwise distances
		(xyLoc & startLoc, xyLoc & goalLoc, subgoalId & start, subgoalId & goal, std::vector<subgoalId> & startSafeConnections, std::vector<subgoalId> & goalSafeConnections, std::vector<subgoalId> & path);
...
private:
	...
	char pairwiseUsage;	// 0: No pairwise distances 1: Pairwise distances 2: Also pairwise connections (not used)

	unsigned int height, width, mapSize, nSubgoals, nGlobalSubgoals;
